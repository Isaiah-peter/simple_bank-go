...
const getEntriesByAccountID = `-- name: GetEntriesByAccountID :one
SELECT id, account_id, amount, created_at FROM entries
WHERE account_id = $1 LIMIT 1
`

func (q *Queries) GetEntriesByAccountID(ctx context.Context, accountID int64) (Entry, error) {
	row := q.db.QueryRowContext(ctx, getEntriesByAccountID, accountID)
	var i Entry
	err := row.Scan(
		&i.ID,
		&i.AccountID,
...
func (q *Queries) ListEntriesByAccountID(ctx context.Context, arg ListEntriesByAccountIDParams) ([]Entry, error) {
	rows, err := q.db.QueryContext(ctx, listEntriesByAccountID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
