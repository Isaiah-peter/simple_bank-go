...
const getTransfersByAccountID = `-- name: GetTransfersByAccountID :one
SELECT id, from_account_id, to_account_id, amount, created_at FROM transfers
...
func (q *Queries) GetTransfersByAccountID(ctx context.Context, arg GetTransfersByAccountIDParams) (Transfer, error) {
	row := q.db.QueryRowContext(ctx, getTransfersByAccountID, arg.ToAccountID, arg.FromAccountID)
	var i Transfer
	err := row.Scan(
		&i.ID,
		&i.FromAccountID,
...
func (q *Queries) ListTransfersByAccountID(ctx context.Context, arg ListTransfersByAccountIDParams) ([]Transfer, error) {
	rows, err := q.db.QueryContext(ctx, listTransfersByAccountID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
